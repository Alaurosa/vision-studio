...
- **Project vision** in **`ProjectVisionIntake.jsx`**: chatbot-first intake at `/vision` (quick mood chips + optional chat input, project spaces as context). Persists normalized `globalVision` (`summary`, `moodTags`, `priorities`, `constraints`, `notes`, `spacesContext`) via `projectVision.js` — no repeated append to `propertyVision`. **Confirmation** sets `confirmationCompletedAt` and opens the editor when appropriate. AI unavailable → toast + deterministic fallback in-thread.
