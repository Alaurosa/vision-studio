...
/**
 * Interior rendering: floor always; optional perimeter walls, wallpaper, and wall art.
 * @param {{ interior: unknown, roomW: number, roomD: number, roomH: number, showWalls?: boolean }} props
 */
export default function RoomInterior3D({ interior, roomW, roomD, roomH, showWalls = false }) {
  ...
  const innerMaterial = useWallpaperMaterial(
    showWalls ? config.wallpaperId : null,
    paintColor,
  );
  ...
  useEffect(() => {
    if (!showWalls) return undefined;
    return () => shellMaterial.dispose();
  }, [showWalls, shellMaterial]);

  const repeatX = Math.max(2, roomW / 1.2);
  const repeatY = Math.max(2, roomH / 1.0);

  useEffect(() => {
    if (!showWalls || !innerMaterial.map) return;
    innerMaterial.map.repeat.set(repeatX, repeatY);
    innerMaterial.map.needsUpdate = true;
  }, [showWalls, innerMaterial, repeatX, repeatY]);
